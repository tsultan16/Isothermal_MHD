...
!
!                     The 1D TVD fluxes are computed using the 2nd order (via flux-limiters) Roe-type Riemann 
!                     solver in K98.
!
!-------------------------------------------------------------------------------------------------------------
...
SUBROUTINE allocate_local_variables()
    ...
    ALLOCATE(Fs(1-nb:nmax+nb,1-nb:nmax+nb,1-nb:nmax+nb, 7,3))
    ALLOCATE(Fs_buff(1-nb:nmax+nb,1-nb:nmax+nb,1-nb:nmax+nb,7,3))
...
SUBROUTINE solve_3d()
    ...
    CALL compute_xfluxes_3d(1, nx, 1, ny, 1, nz, .FALSE.)  ! send false flag since this is the predictor stage

    IF(print_debug) PRINT*,'Computing predictor TVD y-fluxes.'
    
    CALL compute_yfluxes_3d(1, nx, 1, ny, 1, nz, .FALSE.)  ! send false flag since this is the predictor stage

    IF(print_debug) PRINT*,'Computing predictor TVD z-fluxes.'
    
    CALL compute_zfluxes_3d(1, nx, 1, ny, 1, nz, .FALSE.)  ! send false flag since this is the predictor stage
    ...
    CALL compute_cell_corner_emf_3d(1, nx, 1, ny, 1, nz)
    ...
    CALL update_CT_3d(1, nx, 1, ny, 1, nz, .FALSE.) ! send false flag since this is the predictor stage
    ...
    CALL compute_xfluxes_3d(3, nx-2, 3, ny-2, 3, nz-2, .TRUE.)     ! send true flag since this is the corrector stage
    ...
    CALL compute_yfluxes_3d(3, nx-2, 3, ny-2, 3, nz-2, .TRUE.)     ! send true flag since this is the corrector stage    
    ...
    CALL compute_zfluxes_3d(3, nx-2, 3, ny-2, 3, nz-2, .TRUE.)     ! send true flag since this is the corrector stage
    ...
    CALL compute_cell_corner_emf_3d(3, nx-2, 3, ny-2, 3, nz-2)
    ...
    CALL update_CT_3d(3, nx-2, 3, ny-2, 3, nz-2, .TRUE.) ! send true flag since this is the corrector stage
...
SUBROUTINE halfupdate_x_3d(Flux)
   ...
   REAL*8, INTENT(INOUT) :: Flux(1-nb:nmax+nb,1-nb:nmax+nb,1-nb:nmax+nb,7,3)
   INTEGER :: i,j,k
   ...
   DO k = 1-nb+2, nz+nb-2
       DO j = 1-nb+2, ny+nb-2
           DO i = 1-nb+2, nx+nb-2
                ...
                !*******************************************************************************
                !q_3d(i,j,k,6) = q_3d(i,j,k,6) - dtdx * ( Flux(i,j,k,6,2) - Flux(i,j-1,k,6,2) + &
                !                Flux(i,j,k,6,3) - Flux(i,j,k-1,6,3)) + dtdx * sBy
                !q_3d(i,j,k,7) = q_3d(i,j,k,7) - dtdx * ( Flux(i,j,k,7,2) - Flux(i,j-1,k,7,2) + &
                !                Flux(i,j,k,7,3) - Flux(i,j,k-1,7,3)) + dtdx * sBz
                !********************************************************************************
   ...
   DO k = 1-nb+2, nz+nb-2
       DO j = 1-nb+2, ny+nb-2
           DO i = 1-nb+2, nx+nb-2
                ...
                !**********************************************************************************
                !q_3d(i,j,k,5) = q_3d(i,j,k,5) - dtdx * ( Flux(i,j,k,5,1) - Flux(i-1,j,k,5,1) + &
                !                Flux(i,j,k,5,3) - Flux(i,j,k-1,5,3)) + dtdx * sBx
   ...
   DO k = 1-nb+2, nz+nb-2
       DO j = 1-nb+2, ny+nb-2
           DO i = 1-nb+2, nx+nb-2
                ...
                !************************************************************************************************
                !q_3d(i,j,k,5) = q_3d(i,j,k,5) - dtdx * ( Flux(i,j,k,5,1) - Flux(i-1,j,k,5,1) + &
                !                Flux(i,j,k,5,2) - Flux(i,j-1,k,5,2)) + dtdx * sBx
                !q_3d(i,j,k,6) = q_3d(i,j,k,6) - dtdx * ( Flux(i,j,k,6,1) - Flux(i-1,j,k,6,1) + &
                !                Flux(i,j,k,6,2) - Flux(i,j-1,k,6,2)) + dtdx * sBy 
                !************************************************************************************************                
...
SUBROUTINE update_CT_3d(ilow, ihi, jlow, jhi, klow, khi, corrector_stage)
    ...
    IF(corrector_stage) THEN
        ...
        DO k = klow-nb+3, khi+nb-2
            DO j = jlow-nb+3, jhi+nb-2
                DO i = ilow-nb+3, ihi+nb-2  
