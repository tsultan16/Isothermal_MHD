...
SUBROUTINE init_state()
    ...
    INTEGER, PARAMETER :: dir = 2 ! 1 : x, 2 : y, 3 : z , 4: ?  
...
SUBROUTINE boundary_conditions()
    ...
    ! Open Boundaries
        ...
        !DO i = 0, nb-2            
        !    emf_corner(1-nb+i,:,:,:) = emf_corner(0,:,:,:)     
        !END DO


        !DO i = 0, 3        
            !bface_3d(1-nb+i,:,:,1) = bface_3d(0,:,:,1)     
        !    bface_3d(1-nb+i,:,:,:) = bface_3d(0,:,:,:)     
        !END DO
  
        !DO i = 0, 4        
            !bface_3d(nx+1+i,:,:,1) = bface_3d(nx,:,:,1)
        ...
        !DO j = 0, nb-2
        !   emf_corner(:,1-nb+j,:,:) = emf_corner(:,0,:,:)       
        !END DO
        
        !DO j = 0, 3        
            !bface_3d(:,1-nb+j,:,2) = bface_3d(:,0,:,2)     
        !    bface_3d(:,1-nb+j,:,:) = bface_3d(:,0,:,:)     
        !END DO
  
        !DO j = 0, 4        
            !bface_3d(:,ny+1+j,:,2) = bface_3d(:,ny,:,2)
        ...
        !DO k = 0, nb-2
        !    emf_corner(:,:,1-nb+k,:) = emf_corner(:,:,0,:)        
        !END DO
        
        !DO k = 0, 3        
            !bface_3d(:,:,1-nb+k,3) = bface_3d(:,:,0,3)     
        !    bface_3d(:,:,1-nb+k,:) = bface_3d(:,:,0,:)     
        !END DO
  
        !DO k = 0, 4        
            !bface_3d(:,:,nz+1+k,3) = bface_3d(:,:,nz,3)
