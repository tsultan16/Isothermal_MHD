...
REAL*8, ALLOCATABLE :: q(:,:,:),       &      ! stave-vector work array
...
SUBROUTINE deallocate_local_variables()
    ...
    DEALLOCATE(q, Fs, Fs_buff, qavg, Rk, ck, eigenvalues, wave_speeds, emf_2d, emf_corner)
...
SUBROUTINE halfupdate_x_2d()
   ...
   DO j = -1, ny+1
        DO i = -1, nx+1
            ...
            dBx   = bface_2d(i,j,1) - bface_2d(i-1,j,1)
            smomx = q(i,j,5) * dBx
            smomy = q(i,j,6) * dBx
            smomz = q(i,j,7) * dBx
   ...
   idx = 1.d0 / dx
    ...
    DO j = -1, ny+1
        DO i = -1, nx+1
            ...
            ! set the multi-dimensinal source terms
            dBy   = bface_2d(i,j,2) - bface_2d(i,j-1,2)
            smomx = q(j,i,5) * dBy
            smomy = q(j,i,6) * dBy
            smomz = q(j,i,7) * dBy
...
SUBROUTINE compute_xfluxes_2d(corrector_stage) 
    ...
    IF(.NOT. corrector_stage) THEN
        ...
        DO j = -1, ny+1
            DO i = -1, nx+1
                emf_2d(i,j,1) = Fs_buff(i,j,6,1)   ! this is Ez_i+1/2,j 
            END DO
        END DO    
    ...
    ELSE
        ...
        DO j = -1, ny+1
            DO i = -1, nx+1
                emf_2d(i,j,1) = Fs(i,j,6,1)   ! this is Ez_i+1/2,j 
            END DO
        END DO    
    ...
    ELSE
        ...
        DO j = -1, ny+1
            DO i = -1, nx+1     
                ...
                emf_2d(i,j,3) = q_2d(i,j,5) * (q_2d(i,j,3)/q_2d(i,j,1))  - q_2d(i,j,6) * (q_2d(i,j,2)/q_2d(i,j,1))       
...
SUBROUTINE compute_cell_corner_emf_2d()
    ...
    DO j = -1, ny
        DO i = -1, nx
            ...
            vy = 0.5d0 * (q_2d(i,j,3)/q_2d(i,j,1) + q_2d(i+1,j,3)/q_2d(i+1,j,1) )
            ...
            dxL_14_11 = idx2 * (emf_2d(i,j+1,1) - emf_2d(i,j+1,3))             
            ...
            emf_corner(i,j) =  0.25d0 * (emf_2d(i,j,1) + emf_2d(i,j,2) + emf_2d(i,j+1,1) + emf_2d(i+1,j,2)) + &
                               0.0125d0 * dx * ((dy_12_14 - dy_12_34) + (dx_14_12 - dx_34_12)) 
...
SUBROUTINE update_CT(corrector_stage)
    ...
    ! by averageing over the cell-face values
    IF(corrector_stage) THEN
...
SUBROUTINE compute_eigenvalues_1d(ilow, ihi, jlow, jhi)
    ...
    REAL*8 :: cf, ca, cs, bsqr, btsqr, tmp1, tmp2, isrho
    INTEGER :: i, j
    ...
    DO i = -3, ihi+2
        ...
        bsqr  = qavg(i,j,5)**2 + btsqr        
        tmp1  = asqr + bsqr        
        tmp2  = SQRT( (asqr - bsqr)**2 + 4.d0 * asqr * btsqr )

        ca = ABS(qavg(i,j,5))  ! alfven speed
...
SUBROUTINE compute_eigenvectors_1d(ilow, ihi, jlow, jhi)
    ...
    DO i = -3, ihi+2
        ...
        IF(bt .LT. ZERO .AND. ABS(asqr - wave_speeds(i,j,2)**2) .LT. ZERO) THEN
            alphaf(i,j) = 1.D0
            alphas(i,j) = 1.D0
        ELSE
            tmp1 = 1.D0 / SQRT(wave_speeds(i,j,3)**2 - wave_speeds(i,j,1)**2)
            alphaf(i,j) = SQRT(wave_speeds(i,j,3)**2 - wave_speeds(i,j,2)**2) * tmp1       
            alphas(i,j) = SQRT(wave_speeds(i,j,3)**2 - asqr) * tmp1
        END IF
    ...
    DO i = -2, ihi+1
        ...
        IF(amc .LT. ZERO) THEN
            ...
            IF(qavg(i,j,6) .GE. ZERO .AND. qavg(i,j,7) .GT. ZERO) THEN
                signbt = 1.d0
            ELSE IF(qavg(i,j,6) .LE. ZERO .AND. qavg(i,j,7) .LT. ZERO) THEN
               signbt = -1.D0
            ELSE
               signbt = 1.d0            
            END IF
