...
REAL*8, ALLOCATABLE :: q_2d(:,:,:), &    ! vector of conserved variables
                       bface_2d(:,:,:)   ! cell face magnetic field
...
SUBROUTINE create_grid_arrays()
	...
	ALLOCATE(q_2d(1-nb:nx+nb, 1-nb:ny+nb,7))
	ALLOCATE(bface_2d(1-nb:nx+nb, 1-nb:ny+nb,2))
    
    q_2d = 0.d0
    bface_2d = 0.d0
    
    total_grid_memory = SIZEOF(q_2d)  + SIZEOF(bface_2d)
