...
LOGICAL, PARAMETER :: primitive_reconstruction = .FALSE.  ! swtch for using primitive variable reconstruction rather than the default characteristic variable reconstruciton
...
SUBROUTINE allocate_local_variables()
    ...
    total_grid_memory = SIZEOF(qintL) + SIZEOF(qintR) +  SIZEOF(w) +  SIZEOF(qwork_3d) + SIZEOF(bface_work) + &
...
SUBROUTINE vanleer_solve_3d(dt_in, drive_next)
    ...
    !***********************************************************************
    !IF(myrank .EQ. 0 .AND. drive_turbulence .AND. drive_next)   
    ...
    IF(drive_turbulence .AND. drive_next) THEN
        !IF(myrank .EQ. 0) THEN
        !    PRINT*,'##############################################'        
        !    PRINT*, 'Applying corrector turbulence driving force.'
        !    PRINT*,'##############################################'
        !END IF    
        !CALL get_rho_turb(.TRUE.)
        !CALL get_rho_turb(.FALSE.)
        !CALL add_random_force(dt, delv_re, .TRUE.)
        CALL add_random_force(dt, delv_re, delv_max, .FALSE.)
    END IF   
        ...
        IF(.NOT. corrector_stage) THEN
            ...
            DO j = jlow-nb, jhi+nb 
                DO i = ilow-nb, ihi+nb-1        
                    ...
                    !IF(CT_off) qint(i,j,5,1) = 0.5d0*(q_3d(i,j,k,5)+q_3d(i+1,j,k,5))     
                    ...
                    !IF(CT_off) qint(i,j,5,2) = qint(i,j,5,1) 
        ...
        ELSE
            ...
            DO j = jlow-nb, jhi+nb 
                DO i = ilow-nb, ihi+nb                     
                    ...
                    !IF(CT_off) w(i,j,5) = q_3d(i+1,j,k,5)     
        ...
        IF(.NOT. corrector_stage) THEN
            ...
            DO i = ilow-nb, ihi+nb 
                DO j = jlow-nb, jhi+nb-1
                    ...
                    !IF(CT_off) qint(j,i,5,1) = 0.5d0*(q_3d(i,j,k,6)+q_3d(i,j+1,k,6))              !
                    ...
                    !IF(CT_off) qint(j,i,5,2) = qint(j,i,5,1)         
        ...
        ELSE
            ...
            DO i = ilow-nb, ihi+nb 
                DO j = jlow-nb, jhi+nb                     
                    ...
                    w(j,i,5) = bface_3d(i,j,k,2)
                    !IF(CT_off) w(j,i,5) = q_3d(i,j,k,6) 
        ...
        IF(.NOT. corrector_stage) THEN
            ...
            DO i = ilow-nb, ihi+nb 
                DO k = klow-nb, khi+nb-1
                    ...
                    !IF(CT_off) qint(k,i,5,1) = 0.5d0*(q_3d(i,j,k,7)+q_3d(i,j,k+1,7)) 
                    ...
                    !IF(CT_off) qint(k,i,5,2) = qint(k,i,5,1) 
        ...
        ELSE
            ...
            DO i = ilow-nb, ihi+nb 
                DO k = klow-nb, khi+nb                   
                    ...
                    w(k,i,5) = bface_3d(i,j,k,3) 
                    !IF(CT_off) w(k,i,5) = q_3d(i,j,k,7)   
    ...
    DO j = jlow-nb, jhi+nb 
        DO i = ilow-nb, ihi+nb
            ...
            IF(ABS(cf**2-cs**2) .LT. ZERO) THEN
                ...
                alphas = 0.d0 
            ELSE
                alphaf = SQRT(MAX((asqr - cs**2) / (cf**2 - cs**2), 0.d0))
                alphas = SQRT(MAX((cf**2 - asqr) / (cf**2 - cs**2), 0.d0))
            END IF
            
            IF(btsqr .LT. ZERO) THEN
                betay = 1.D0 / SQRT(2.D0)
                betaz = 1.D0 / SQRT(2.D0)
            ELSE
                betay = by / SQRT(btsqr)
                betaz = bz / SQRT(btsqr)
            END IF
...
SUBROUTINE compute_interface_states(ilow, ihi, jlow, jhi)
    ...
    INTEGER, INTENT(IN) :: ilow, ihi, jlow, jhi
    INTEGER :: i, j, k
    REAL*8 :: delw_L(6), delw_R(6), delw_C(6), dela_L(6), dela_R(6), dela_C(6), dela_m(6), delw(1-nb:nmax+nb,1-nb:nmax+nb,6)
    ...
    DO j = jlow-nb, jhi+nb
        DO i = ilow-nb+1, ihi+nb-1
            ...
            ! Compute (cell-center) left, right and centered differences of the primitive variables 
            delw_L(1:4) = w(i,j,1:4) - w(i-1,j,1:4) 
            delw_L(5:6) = w(i,j,6:7) - w(i-1,j,6:7) 
            delw_R(1:4) = w(i+1,j,1:4) - w(i,j,1:4) 
            delw_R(5:6) = w(i+1,j,6:7) - w(i,j,6:7) 
            delw_C(:) = 0.5d0 *( delw_L(:) + delw_R(:) )
    
         
            !  Project differences of the primitive variables onto characteristic variables          
            DO k = 1, 6
            
                dela_L(k) = Lw(i,j,k,1) * delw_L(1) + Lw(i,j,k,2) * delw_L(2) + Lw(i,j,k,3) * delw_L(3) + & 
                            Lw(i,j,k,4) * delw_L(4) + Lw(i,j,k,5) * delw_L(5) + Lw(i,j,k,6) * delw_L(6) 
            
                dela_R(k) = Lw(i,j,k,1) * delw_R(1) + Lw(i,j,k,2) * delw_R(2) + Lw(i,j,k,3) * delw_R(3) + & 
                            Lw(i,j,k,4) * delw_R(4) + Lw(i,j,k,5) * delw_R(5) + Lw(i,j,k,6) * delw_R(6) 
            
                !dela_C(k) = Lw(i,j,k,1) * delw_C(1) + Lw(i,j,k,2) * delw_C(2) + Lw(i,j,k,3) * delw_C(3) + & 
                !            Lw(i,j,k,4) * delw_C(4) + Lw(i,j,k,5) * delw_C(5) + Lw(i,j,k,6) * delw_C(6) 
            ...
            DO k = 1, 6
                ...
                !dela_m(k) = 0.5d0 * (SIGN(1.d0, dela_L(k)) + SIGN(1.d0, dela_R(k))) * &
                !             MIN( 2.d0 * ABS(dela_L(k)), 2.d0 * ABS(dela_R(k)), ABS(dela_C(k)) )
    
                !dela_m(k) = SIGN(1.d0, dela_C(k)) * MIN( 2.d0 * ABS(dela_L(k)), 2.d0 * ABS(dela_R(k)), ABS(dela_C(k)) )                
                
                ! min-mod limiter (most stable)
                dela_m(k) =  0.5d0 * (SIGN(1.d0, dela_L(k)) + SIGN(1.d0, dela_R(k))) * MIN(ABS(dela_L(k)), ABS(dela_R(k)))
            ...
            DO k = 1, 6
                ...
                !IF(primitive_reconstruction) delw(i,j,k) =0.5d0 * (SIGN(1.d0, delw_L(k)) + SIGN(1.d0, delw_R(k))) * MIN(ABS(delw_L(k)), ABS(delw_R(k)))
    ...
    DO j = jlow-nb, jhi+nb
        DO i = ilow-nb+1, ihi+nb-2
            ...
            qintL(i,j,5) =  w(i,j,5)   
            qintL(i,j,6) =  w(i,j,6) + 0.5d0 * delw(i,j,5)  
            qintL(i,j,7) =  w(i,j,7) + 0.5d0 * delw(i,j,6)  
    ...
    DO j = jlow-nb, jhi+nb
        DO i = ilow-nb+1, ihi+nb-2
            ...
            qintR(i,j,5) =  w(i,j,5)  
            qintR(i,j,6) =  w(i+1,j,6) - 0.5d0 * delw(i+1,j,5)  
            qintR(i,j,7) =  w(i+1,j,7) - 0.5d0 * delw(i+1,j,6)  
...
SUBROUTINE compute_HLLD_fluxes_1d(Flux, ilow, ihi, jlow, jhi)
   ...
   DO j = jlow-nb, jhi+nb
        ...
        ! left and right states and fluxes
            ...
            !####################################
            ! check for and treat degenrate cases  
            !#################################### 
...
SUBROUTINE get_max_speed_vanleer(max_speed)
    ...
    DO k = 1, nz    
      DO j = 1, ny    
        DO i = 1, nx ! no need to include boundary cells for this
            ...
            cfx = SQRT( 0.5d0 * (asqr + bsqr + tmp) ) ! fast mode velocity x
            ...
            cfy = SQRT( 0.5d0 * (asqr + bsqr + tmp) ) ! fast mode velocity y

            btsqr = bx**2 + by**2 
            bsqr  = bz**2 + btsqr 
            tmp = SQRT( (asqr - bsqr)**2 + 4.d0 * asqr * btsqr )
            
            cfz = SQRT( 0.5d0 * (asqr + bsqr + tmp) ) ! fast mode velocity z
    ...
    max_speed = MAX(max_speed, smax) !smax + delv_max)
