...
INTEGER(KIND=MPI_ADDRESS_KIND) :: disp(26), disp_q(26), disp_b(26) ! 6 faces + 12 edges + 8 corners
INTEGER(KIND=MPI_ADDRESS_KIND) :: shuffled_disp(26), shuffled_disp_b(26) 
INTEGER :: send_buffer_size(26), send_buffer_size_b(26)
INTEGER :: window
...
SUBROUTINE initialize_MPI()
    ...
    ! allocate memory for a general purpose buffer that will be used during MPI communications
    max_buffer_size = MAX(nx*ny*nb, ny*nz*nb, nz*nx*nb, nx*nb*nb, ny*nb*nb, nz*nb*nb, nb*nb*nb) * 10
...
SUBROUTINE do_halo_exchange()

    INTEGER :: i

    ! region of RMA load/stores bounded by fences (active target synchronization)
    ...
    ! maybe do some work here instead of just waiting...
    !CALL MPI_BARRIER(comm3d, ierr)
...
SUBROUTINE do_halo_exchange_2()




END SUBROUTINE do_halo_exchange_2
...
SUBROUTINE send_bndry(bndry)
    ...
    CALL packup_outbuffer(bndry)         
    CALL MPI_PUT(buffer, buff_size, MPI_DOUBLE_PRECISION, neighbor_rank(bndry), &
