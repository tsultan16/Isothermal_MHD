...
INTEGER, PARAMETER :: riemann_solver_type = 2 ! 1: HLLD, 2: Roe_TVD (second order)
...
SUBROUTINE allocate_local_variables()
    ...
    total_grid_memory = SIZEOF(q) + SIZEOF(qwork_3d) + SIZEOF(bface_work) + &
                        ...
                        SIZEOF(wave_speeds) + SIZEOF(emf_3d) + SIZEOF(emf_corner)
...
SUBROUTINE compute_xfluxes_3d(ilow, ihi, jlow, jhi, klow, khi, corrector_stage) 
    ...
    DO k = klow-nb, khi+nb
        ...
        IF(riemann_solver_type .EQ. 1) THEN
            ...
            CALL compute_HLLD_fluxes_1d(Fs_2d, ilow, ihi, jlow, jhi)
        ...
        ELSE IF(riemann_solver_type .EQ. 2) THEN
            ...
            IF(flux_protection) CALL compute_HLLD_fluxes_1d(Fs_2d_prot, ilow, ihi, jlow, jhi)
...
SUBROUTINE compute_yfluxes_3d(ilow, ihi, jlow, jhi, klow, khi, corrector_stage) 
    ...
    DO k = klow-nb, khi+nb
        ...
        IF(riemann_solver_type .EQ. 1) THEN
            ...
            CALL compute_HLLD_fluxes_1d(Fs_2d, jlow, jhi, ilow, ihi)
        ...
        ELSE IF(riemann_solver_type .EQ. 2) THEN
            ...
            IF(flux_protection) CALL compute_HLLD_fluxes_1d(Fs_2d_prot, jlow, jhi, ilow, ihi)
...
SUBROUTINE compute_zfluxes_3d(ilow, ihi, jlow, jhi, klow, khi, corrector_stage) 
    ...
    DO j = jlow-nb, jhi+nb  
        ...
        IF(riemann_solver_type .EQ. 1) THEN
            ...
            CALL compute_HLLD_fluxes_1d(Fs_2d,  klow, khi, ilow, ihi)
        ...
        ELSE IF(riemann_solver_type .EQ. 2) THEN
            ...
            IF(flux_protection) CALL compute_HLLD_fluxes_1d(Fs_2d_prot,  klow, khi, ilow, ihi)
        ...
        DO k = 1-nb+3, nz+nb-3
            DO j = 1-nb+3, ny+nb-3
                DO i = 1-nb+3, nx+nb-3     
                    ...
                    ! instead of using half updated B fields here ,try using the initial B field, check how that affects bfield loop advection...
...
SUBROUTINE compute_HLLD_fluxes_1d(Flux, ilow, ihi, jlow, jhi)
    ...
    REAL*8 :: FL(7), FR(7)
    REAL*8 :: rho_s, vx_s, flux_rho_s, flux_momx_s, irho_s, isrho_s, X, iX 
    ...
    REAL*8 :: tmp1, tmp2, asqr, bsqr, btsqr, ca_sqr, cf, isRL
    ...
    DO i = ilow-nb, ihi+nb-1
        ...
        Bx = 0.5d0 * (uL(5) + uR(5)) ! in 1D, Bx would just be constant. However in >1D, Bx could vary, so I'm taking the average
        ...
        FL(1) = uL(2)
        FL(2) = uL(1) * (vxL**2 + sound_speed**2) + 0.5d0 *(uL(6)**2 + uL(7)**2 - uL(5)**2)
        ...
        FR(1) = uR(2)
        FR(2) = uR(1) * (vxR**2 + sound_speed**2) + 0.5d0 *(uR(6)**2 + uR(7)**2 - uR(5)**2)
        ...
        tmp1 = asqr + bsqr        
        tmp2 = SQRT( (asqr - bsqr)**2 + 4.d0 * asqr * btsqr )
        cf   = SQRT( 0.5d0 * (tmp1 + tmp2) )
        
        lambdaf_minus_L = vxL - cf  
        lambdaf_plus_L  = vxL + cf  
        ...
        lambdaf_minus_R = vxR - cf  
        lambdaf_plus_R  = vxR + cf          

        sL = MIN(lambdaf_minus_L, lambdaf_minus_R)
        sR = MAX(lambdaf_plus_L, lambdaf_plus_R)       
        
        ! star region density, x-velocity and corresponding fluxes
        isRL = 1.d0 / (sR-sL)
        flux_rho_s  = (sR * FL(1) - sL * FR(1) + sR * sL* (uR(1) - uL(1)) ) * isRL
        flux_momx_s = (sR * FL(2) - sL * FR(2) + sR * sL* (uR(2) - uL(2)) ) * isRL      
              
        rho_s = (sR * uR(1) - sL * uL(1) - FR(1) + FL(1) ) * isRL
        vx_s  = flux_rho_s / rho_s
    
        irho_s  = 1.d0 / rho_s
        isrho_s = 1.d0 / SQRT(rho_s)
    
        ! star region inner wave speeds
        sL_s = vx_s - ABS(Bx) * isrho_s     
        sR_s = vx_s + ABS(Bx) * isrho_s    
    
    
        ! check for signal speed consistency
        !IF((sL_s -sL .LT. ZERO) .OR. (sR_s -sR .GT. ZERO)) THEN
        !    PRINT*,'ERROR. Found bad values for HLL interior wave speeds!'
        !    PRINT*,'sL, sL_s, sR_s, sR =',sL, sL_s, sR_s, sR
        !    STOP
        !END IF
    
    
        ! star region left: tangential variables
        IF(ABS(sL - sL_s) .LT. ZERO) THEN
        ...
        ELSE
            ...
            tmp1 = Bx * (vx_s - vxL) * irho_s * isRL
            tmp2 = ( uL(1) * (sL - vxL)**2 - Bx**2 ) * irho_s * isRL
        
            vy_s_L = vyL -  tmp1 * uL(6)
            vz_s_L = vzL -  tmp1 * uL(7)
            By_s_L = uL(6) * tmp2
            Bz_s_L = uL(7) * tmp2
    
        END IF
    
        ! star region right: tangential variables      
        IF(ABS(sR - sR_s) .LT. ZERO) THEN
        ...
        ELSE
            ...
            tmp1 = Bx * (vx_s - vxR) * irho_s * isRL
            tmp2 = ( uR(1) * (sR - vxR)**2 - Bx**2 ) * irho_s * isRL
        
            vy_s_R = vyR -  tmp1 * uR(6)
            vz_s_R = vzR -  tmp1 * uR(7)
            By_s_R = uR(6) * tmp2
            Bz_s_R = uR(7) * tmp2

        END IF
        
        ! star region center: tangential variables
        X  = isrho_s * SIGN(1.d0, Bx)
        iX = 1.d0 / X
        
        vy_s_C = 0.5d0 * ( (vy_s_L + vy_s_R) + X * (By_s_R - By_s_L) )    
        vz_s_C = 0.5d0 * ( (vz_s_L + vz_s_R) + X * (Bz_s_R - Bz_s_L) )    
        By_s_C = 0.5d0 * ( (By_s_L + By_s_R) + iX * (vy_s_R - vy_s_L) )    
        Bz_s_C = 0.5d0 * ( (Bz_s_L + Bz_s_R) + iX * (vz_s_R - vz_s_L) )    
                    
        
        ! Now, we're ready to compute the interface flux based on the star region states                    
        IF(sL .GT. 0.d0) THEN
        
            Flux(i,j,:) = FL(:)
        
        ELSE IF (sL_s .GT. 0.d0) THEN
        
            Flux(i,j,1) = FL(1) + SL * (rho_s - uL(1))
            Flux(i,j,2) = FL(2) + SL * (rho_s * vx_s   - uL(2))
            Flux(i,j,3) = FL(3) + SL * (rho_s * vy_s_L - uL(3))
            Flux(i,j,4) = FL(4) + SL * (rho_s * vz_s_L - uL(4))
            Flux(i,j,5) = 0.d0
            Flux(i,j,6) = FL(6) + SL * (By_s_L - uL(6))
            Flux(i,j,7) = FL(7) + SL * (Bz_s_L - uL(7))
        
        ELSE IF (sR_s .GT. 0.d0) THEN
        
            Flux(i,j,1) = flux_rho_s
            Flux(i,j,2) = flux_momx_s
            Flux(i,j,3) = rho_s * vx_s * vy_s_C - Bx * By_s_C
            Flux(i,j,4) = rho_s * vx_s * vz_s_C - Bx * Bz_s_C
            Flux(i,j,5) = 0.d0
            Flux(i,j,6) = vx_s * By_s_C - vy_s_C * Bx
            Flux(i,j,7) = vx_s * Bz_s_C - vz_s_C * Bx       
        
        
        ELSE IF (sR .GT. 0.d0) THEN

            Flux(i,j,1) = FR(1) + SR * (rho_s - uR(1))
            Flux(i,j,2) = FR(2) + SR * (rho_s * vx_s   - uR(2))
            Flux(i,j,3) = FR(3) + SR * (rho_s * vy_s_R - uR(3))
            Flux(i,j,4) = FR(4) + SR * (rho_s * vz_s_R - uR(4))
            Flux(i,j,5) = 0.d0
            Flux(i,j,6) = FR(6) + SR * (By_s_R - uR(6))
            Flux(i,j,7) = FR(7) + SR * (Bz_s_R - uR(7))
